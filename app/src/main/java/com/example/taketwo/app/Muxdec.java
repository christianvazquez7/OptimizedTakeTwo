...
public class Muxdec {
    ...
    public void offerAudioEncoder(Pair<Long,byte []> input) {
        ...
        if (audioIndex != -1 && trackIndex != -1 && !started) {
            mMuxer.start();
            started = true;
        }
        do
        {
            if (outputBufferIndex >= 0)
            {
                ...
                if(bufferInfo.size != 0 && audioIndex != -1 && started) {
    ...
    public void offerEncoder(Pair<Long,byte[]> input) {
        ...
        do
        {
            ...
            if (outputBufferIndex >= 0)
            {
                ...
                if(bufferInfo.size != 0  && trackIndex != -1 && started) {
    ...
    public byte[] NV21toNV12(byte[] input, int width, int height) {
        ...
        for (int i =0; i<limit ; i++) {
            byte a = input[frameSize + i*2];
            input[frameSize + i*2] = input[frameSize + i*2+1];
            input [frameSize + i*2 +1] = a;
        }
        return input;
    }
}
